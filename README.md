...
Пиковый RPS пересчитывается только для API/Web запросов, проходящих через ingress. Медиа-трафик (просмотры видео и загрузка изображений) обслуживается CDN и не учитывается в расчете нагрузки на L7.

Пиковый RPS API рассчитан без просмотров видео:

* Публикация твитов: 8 073 RPS
* Лайки: 48 610 RPS
* Ретвиты: 5 556 RPS

| Параметр                                   | Значение                       |
| ------------------------------------------ | ------------------------------ |
| Пиковый RPS API (без media)        | 62 239                         |
| Доля новых TLS-соединений                  | 20%                            |
| Пиковый CPS (TLS handshakes)               | 62 239 * 0.20 = 12 448         |
...
| Реальная пиковая пропускная способность L7 | ~50 Гбит/с                     |
...
vCPU_required = ceil(CPS_peak / handshakes_per_vCPU) = ceil(12 448 / 428) = ceil(29.1) = 30 vCPU
...
N_work_ssl = ceil(30 / 16) = ceil(1.875) = 2
...
Количество нод, необходимых для пикового API/Web трафика:

N_net = ceil(50 / 5.87) = ceil(8.52) = 9

Сеть становится ограничителем, потому что:

* API/Web трафик теперь считается отдельно от CDN media traffic.
* Через ingress проходят JSON API, HTML, websocket-соединения, изображения профилей, metadata responses.
* В отличие от предыдущего расчета, media CDN traffic больше не учитывается.

Однако значение 50 Гбит/с является завышенной оценкой с большим запасом. Реальный API/Web трафик существенно ниже из-за:

* keep-alive соединений,
* HTTP/2 multiplexing,
* gzip/br сжатия,
* малого размера JSON-ответов.

Поэтому ограничителем остается CPU на TLS и обработку запросов.

#### Итоговое количество

| Назначение                       | N_work | Схема резервирования | N_total | vCPU на ноду | Итого vCPU |
| -------------------------------- | ------ | -------------------- | ------- | ------------ | ---------- |
| API + Web (`api.x.com`, `x.com`) | 2      | N + 1                | 3       | 16           | 48         |

Итоговое количество L7-балансировщиков: 3 ноды (2 рабочие + 1 резервная).
...
| L7 (NGINX)      | Падение одной ноды (из 3) | Оставшиеся 2 ноды принимают трафик   | менее 30 сек |
